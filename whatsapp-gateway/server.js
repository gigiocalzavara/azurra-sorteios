...
function phoneCandidates(phone){
 const raw=digits(phone),values=new Set([raw]);
 if(raw.startsWith("55")){
  if(raw.length===13&&raw[4]==="9")values.add(raw.slice(0,4)+raw.slice(5));
  if(raw.length===12)values.add(raw.slice(0,4)+"9"+raw.slice(4));
 }
...
async function resolveDirectJid(org,phone){
 ...
 for(const candidate of candidates){
  try{
   const found=await s.sock.onWhatsApp(candidate);
   const item=Array.isArray(found)?found.find(entry=>entry?.exists):null;
   if(item?.jid)return item.jid;
  }catch(error){logger.warn({organization:org,candidate,error:error?.message},"onWhatsApp lookup failed")}
 }
 if(!candidates[0])throw new Error("Telefone do vencedor inválido");
...
async function productMenu(flow){
 ...
 const rows=data||[];
 return {rows,text:`🏆 Parabéns! Você foi o vencedor.\n\nDigite o número do produto que você quer:\n${rows.map((r,i)=>`${i+1}. ${Array.isArray(r.products)?r.products[0]?.name:r.products?.name||"Produto"}`).join("\n")}`};
}
...
async function startPostPurchase(flow){
 ...
 const result=await sendDirectMessage(flow.organization_id,flow.phone_e164,menu.text);
 const {error}=await supabase.from("post_purchase_flows").update({stage:"awaiting_product",started_at:new Date().toISOString(),last_message_id:result?.key?.id,last_error:null,updated_at:new Date().toISOString()}).eq("id",flow.id);
 if(error)throw error;
}

async function findActiveWinnerFlow(org,phone){
 const {data,error}=await supabase.from("post_purchase_flows").select("id,organization_id,promotion_id,phone_e164,stage,selected_product_id,promotions(post_draw_pix_amount)").eq("organization_id",org).in("stage",["awaiting_product","awaiting_reward_type","awaiting_address"]).order("created_at",{ascending:false}).limit(30);
 if(error)throw error;
 return (data||[]).find(flow=>phonesEquivalent(flow.phone_e164,phone))||null;
}
...
async function handleWinnerReply(org,phone,text){
 ...
 if(flow.stage==="awaiting_reward_type"){
  if(answer==="1"){
   ...
   await sendDirectMessage(org,flow.phone_e164,"Obrigado! Registramos sua escolha pelo PIX. Um operador entrará em contato para combinar os dados e realizar o pagamento.");
   return true;
  }
  if(answer==="2"){
   ...
   if(updateError)throw updateError;
   await sendDirectMessage(org,flow.phone_e164,"Perfeito. Envie seu endereço completo para entrega, incluindo rua, número, complemento (se houver), bairro, cidade, estado e CEP.");
   return true;
  }
  await sendDirectMessage(org,flow.phone_e164,"Digite 1 para PIX ou 2 para Produto.");return true;
 }
 if(flow.stage==="awaiting_address"){
  if(answer.length<10){await sendDirectMessage(org,flow.phone_e164,"Por favor, envie o endereço completo para conseguirmos organizar a entrega.");return true;}
  ...
  if(updateError)throw updateError;
  await sendDirectMessage(org,flow.phone_e164,"Obrigado! Recebemos seu endereço. Agora nossa equipe vai organizar o envio do seu prêmio.");
  return true;
 }
 return false;
}
...
async function processQueue(onlyOrg=null){
 ...
 try{
  ...
  let flowQuery=supabase.from("post_purchase_flows").select("id,organization_id,promotion_id,phone_e164,stage").in("stage",["pending_send","failed"]).order("updated_at").limit(10);
