...
export default async function Page({searchParams}:{searchParams:Promise<{saved?:string;error?:string;membersaved?:string;memberremoved?:string}>}){
 ...
 return <main className={styles.main}>
  <div className={styles.heading}><div><h1>Configurações</h1><p>Identidade da operação, PIX e acessos da equipe.</p></div></div>
  ...
  {org?<>
   <form className={styles.form} action={saveSettings}>
    <section className={styles.section}><h2>Empresa e identidade</h2><div className={styles.fields}>
      <label className={styles.field}><span>Nome da operação</span><input name="name" required defaultValue={org.name}/></label>
      <label className={styles.field}><span>Cor principal</span><input name="primaryColor" type="color" defaultValue={org.primary_color}/></label>
      <label className={styles.field}><span>Cor secundária</span><input name="secondaryColor" type="color" defaultValue={org.secondary_color}/></label>
    </div></section>
    <section className={styles.section}><h2>Recebimento via PIX</h2><p>Esses dados aparecem no fluxo de pagamento das reservas.</p><div className={styles.fields}>
      <label className={styles.field}><span>Tipo de chave</span><select name="pixKeyType" required defaultValue={org.pix_key_type||"phone"}><option value="phone">Telefone</option><option value="cpf">CPF</option><option value="cnpj">CNPJ</option><option value="email">E-mail</option><option value="random">Chave aleatória</option></select></label>
      <label className={styles.field}><span>Chave PIX</span><input name="pixKey" required defaultValue={org.pix_key||""}/></label>
      <label className={styles.field}><span>Nome do recebedor</span><input name="receiverName" required maxLength={25} defaultValue={org.pix_receiver_name||""}/></label>
      <label className={styles.field}><span>Cidade do recebedor</span><input name="receiverCity" required maxLength={15} defaultValue={org.pix_receiver_city||""}/></label>
    </div></section>
   ...
   {canManageUsers?<section className={styles.section} style={{marginTop:24}}>
    <h2>Usuários e permissões</h2><p>Adicione usuários já cadastrados e defina o nível de acesso.</p>
    ...
    <div style={{display:"grid",gap:10,marginTop:20}}>
      {(members as MemberRow[]||[]).map(row=><article key={row.user_id} className={styles.memberCard}>
        <div><strong>{row.full_name||row.email}</strong><div className={styles.memberMeta}>{row.email} · {roleLabel[row.role]}</div></div>
        <div className={styles.memberActions}>
          <form action={updateMemberRole}><input type="hidden" name="userId" value={row.user_id}/><select name="role" defaultValue={row.role} disabled={row.user_id===auth.user!.id&&!profile?.is_superadmin}><option value="admin">Administrador</option><option value="manager">Gerente</option><option value="operator">Operador</option>{profile?.is_superadmin?<option value="superadmin">Superadmin</option>:null}</select><button disabled={row.user_id===auth.user!.id&&!profile?.is_superadmin}>Alterar</button></form>
