...
type Contact = { jid: string; phone: string; name: string | null; admin: string | null };
...
export default function LeadCapture({ organizationId }: { organizationId: string }) {
  ...
  const loadContacts = async () => { if (!groupId) return; setBusy(true); setMessage("Lendo participantes do grupo..."); try { const data = await call(`/api/lead-capture?org=${organizationId}&action=participants&groupId=${encodeURIComponent(groupId)}`); setContacts(data.contacts); setSelected(new Set(data.contacts.map((contact: Contact) => contact.phone))); setMessage(`${data.contacts.length} contatos encontrados.`); } catch (error) { setMessage(error instanceof Error ? error.message : "Falha"); } finally { setBusy(false); } };
  const toggle = (contactPhone: string) => setSelected((current) => { const next = new Set(current); next.has(contactPhone) ? next.delete(contactPhone) : next.add(contactPhone); return next; });
  const importContacts = async () => { if (!group) return; setBusy(true); try { const chosen = contacts.filter((contact) => selected.has(contact.phone)); const result = await call("/api/lead-capture", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ org: organizationId, action: "import", groupId: group.id, groupName: group.subject, foundCount: contacts.length, contacts: chosen }) }); setMessage(`${result.imported} importados: ${result.new} novos e ${result.duplicates} já existentes.`); setTimeout(() => location.reload(), 900); } catch (error) { setMessage(error instanceof Error ? error.message : "Falha"); } finally { setBusy(false); } };
  ...
  return <section className={local.module}>
    ...
    {contacts.length ? <div className={local.preview}><div className={local.previewHead}><label><input type="checkbox" checked={allSelected} onChange={(event) => setSelected(event.target.checked ? new Set(contacts.map((contact) => contact.phone)) : new Set())} /> Selecionar todos</label><strong>{selected.size} de {contacts.length} selecionados</strong></div>
      <div className={local.list}>{contacts.map((contact) => <label key={contact.jid}><input type="checkbox" checked={selected.has(contact.phone)} onChange={() => toggle(contact.phone)} /><span><strong>{contact.name || "Nome não disponível"}</strong><small>{contact.phone}{contact.admin ? " • administrador" : ""}</small></span></label>)}</div>
