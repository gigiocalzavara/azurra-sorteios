...
type Props={drawId:string;promotionId:string;promotion:string;imageUrl:string|null;quotaQuantity:number;winningNumber:number;winnerName:string;organization:string;logoUrl:string|null;primaryColor:string;secondaryColor:string};
export default function DrawExperience(props:Props){
 ...
 const record=async()=>{try{const captured=await navigator.mediaDevices.getDisplayMedia({video:{displaySurface:"browser"},audio:false});stream.current=captured;const preferred=["video/mp4;codecs=avc1","video/webm;codecs=vp9","video/webm;codecs=vp8","video/webm"].find(t=>MediaRecorder.isTypeSupported(t));const chunks:Blob[]=[];const mediaRecorder=new MediaRecorder(captured,preferred?{mimeType:preferred}:undefined);recorder.current=mediaRecorder;mediaRecorder.ondataavailable=e=>{if(e.data.size)chunks.push(e.data)};mediaRecorder.onstop=async()=>{const type=mediaRecorder.mimeType||"video/webm";const extension=type.includes("mp4")?"mp4":"webm";const blob=new Blob(chunks,{type});const url=URL.createObjectURL(blob);const link=document.createElement("a");link.href=url;link.download=`resultado-${props.promotion.toLowerCase().replace(/[^a-z0-9]+/g,"-")}.${extension}`;link.click();setRecording(false);setMessage("Salvando o vídeo na promoção...");const supabase=createClient();const path=`${props.promotionId}/${props.drawId}.${extension}`;const {error}=await supabase.storage.from("draw-videos").upload(path,blob,{contentType:type,upsert:true});if(error){setMessage(`Vídeo baixado, mas não foi salvo no sistema: ${error.message}`)}else{const {data:publicFile}=supabase.storage.from("draw-videos").getPublicUrl(path);await Promise.all([supabase.from("draws").update({video_url:publicFile.publicUrl}).eq("id",props.drawId),supabase.from("communication_events").update({media_url:publicFile.publicUrl,updated_at:new Date().toISOString()}).eq("promotion_id",props.promotionId).eq("stage","result")]);setMessage("Vídeo gravado, baixado e anexado à mensagem de resultado.")}setTimeout(()=>URL.revokeObjectURL(url),1000)};captured.getVideoTracks()[0].onended=()=>{if(mediaRecorder.state!=="inactive")mediaRecorder.stop()};mediaRecorder.start(250);setRecording(true);run();setTimeout(()=>{if(mediaRecorder.state!=="inactive")mediaRecorder.stop();captured.getTracks().forEach(track=>track.stop())},11500)}catch{setMessage("A gravação foi cancelada. Selecione esta guia quando o navegador perguntar o que compartilhar.")}};
