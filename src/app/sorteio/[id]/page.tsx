...
export default async function DrawPage({params}:{params:Promise<{id:string}>}){
 ...
 return <DrawExperience drawId={draw?.id||null} promotionId={promotionId} organizationId={promotion.organization_id} promotion={promotion.name} imageUrl={promotion.product_image_url} quotaQuantity={promotion.quota_quantity} winningNumber={quota?.number||null} winnerName={participant.name||null} resultUrl={participant.result_url||null} organization={organization?.name||"Sorteio"} logoUrl={organization?.logo_url} primaryColor={organization?.primary_color||"#6900ff"} secondaryColor={organization?.secondary_color||"#00f0ff"}/>;
}
