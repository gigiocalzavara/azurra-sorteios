...
export function PaymentClient({token}:{token:string}){
 ...
 if(!order)return <main className={styles.page}><section className={styles.card}><ShieldCheck className={styles.brand}/><h1>Acessar pagamento</h1><p>Informe o mesmo WhatsApp usado na reserva.</p><form onSubmit={validate}><label htmlFor="phone">WhatsApp</label><input id="phone" name="phone" inputMode="tel" required placeholder="(83) 99999-9999"/><button>Continuar</button></form>{error?<div className={styles.error}>{error}</div>:null}</section></main>;
 const reported=order.status==="payment_reported";
 return <main className={styles.page}><section className={styles.card}><span className={styles.eyebrow}>Pagamento da reserva</span><h1>{order.promotion.name}</h1><div className={styles.numbers}>{order.quota_numbers.map(n=><span key={n}>{n}</span>)}</div><div className={styles.total}><span>Total via PIX</span><strong>{money(Number(order.total_amount))}</strong></div>{payload?<><div className={styles.qr}><QRCodeSVG value={payload} size={210} level="M"/><small>Leia o QR Code no aplicativo do seu banco</small></div><label>PIX copia e cola</label><button className={styles.copy} type="button" onClick={copy}><span>{payload}</span>{copied?<Check/>:<Copy/>}</button></>:<div className={styles.error}>A chave PIX ainda não foi configurada pelo responsável.</div>}<button className={styles.pay} type="button" onClick={report} disabled={loading||reported||!payload}>{reported?<><Check/> Pagamento informado</>:loading?<LoaderCircle className={styles.spin}/>:"Já fiz o pagamento"}</button>{reported?<p className={styles.success}>Aviso enviado. Aguarde a confirmação do responsável.</p>:null}{error?<div className={styles.error}>{error}</div>:null}</section></main>
}
